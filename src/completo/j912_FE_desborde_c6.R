...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ cliente_antiguedad==1 , ctrx_quarter_normalizado := ctrx_quarter * 6 ] #JP corrijo calculo raro
  ...
  #dataset[ , borrarejemplo             := mv_mpagado + mv_mlimitecompra ]
  dataset[ , jeipi_trax_varias                  := ccallcenter_trx *100 + chomebanking_trx + ccajas_consultas *90 + cmobile_app_trx *80]
  ...
  dataset[ , antiguedad_vs_edad      := cliente_antiguedad / (cliente_edad-18) ]
  dataset[ , inv_total  := minversion1_pesos+minversion2]
  dataset[  , ratio_inversiones_saldos  := (minversion1_pesos+minversion2) / mcuentas_saldo ]
  dataset[ , campos_nulos  := rowSums( is.na(dataset) ) ]
  dataset[ , delta_cheques  := ccheques_emitidos-ccheques_depositados]
  dataset[ , promedio_extraccion  := mextraccion_autoservicio/cextraccion_autoservicio]
  dataset[ , descuentos_totales  := mcajeros_propios_descuentos+mtarjeta_visa_descuentos+mtarjeta_master_descuentos]
  dataset[ , debitos_automaticos  := ccuenta_debitos_automaticos+ctarjeta_visa_debitos_automaticos+ctarjeta_master_debitos_automaticos]
...
TendenciaYmuchomas  <- function( dataset, cols, ventana=4, tendencia=TRUE, minimo=TRUE, maximo=TRUE, promedio=TRUE, 
